...
uses Data, Edit, UnitSprav, FastReport;
...
begin
  ...
  if title = 'имя' then title := 'name';
  if title = 'фамилия' then title := 'surname';
  if title = 'отчество' then title := 'fathername';
  if title = 'дата рождения' then title := 'birthdate';
  if title = 'факультет' then title := 'faculty_idfaculty';
...
begin
  ...
  DBNavigator1.BtnClick(nbRefresh);
    if Data.DataModule2.MainTable.RecordCount = 0 then
      begin
        ShowMessage('Добавьте сотрудника в таблицу');
      end
      else
      begin
        Form4.ShowModal();
...
begin
   ...
   Form5.frxReport1.Export(Form5.frxDOCXExport1);
end;
...
begin
    Data.DataModule2.ADOConnection1.Connected:=true;
      ...
      DataModule2.NationalityTable.Active:=True;
